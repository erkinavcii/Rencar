...
### Araç Durumu (BEFORE) Ekranı — Geri Tuşu Artık PREPARING Kiralamayı İptal Ediyor (Düzeltme)

- Karar: Kullanıcı bildirdi: araç seçip rezervasyonu tamamladıktan sonra Araç Durumu (BEFORE, sürüş öncesi foto) ekranında hiç foto yüklemeden geri tuşuna basınca, kiralama backend'de yarım kalmış durumda (foto tamamlanmadığı için `startRental` hiç başarıyla tetiklenmemiş `PREPARING`) asılı kalıyordu; ekran yalnızca `popBackStack()` çağırıyordu, iptal ile ilgili hiçbir çağrı yapılmıyordu. `cancelPreparingRental` (`DELETE /rentals/{id}`, yalnız `PREPARING` durumundaki kiralama için geçerli) veri katmanında (`RentalRepository.kt:22`) zaten tanımlıydı ama hiçbir ekrandan çağrılmıyordu.

- Son Güncelleme Tarihi: 19.07.2026

- Uygulama: `VehicleConditionContract.kt`'ye `VehicleConditionIntent.BackClicked` ve `VehicleConditionEffect.NavigateBack` eklendi. `VehicleConditionViewModel.onBackClicked()`: `mode == BEFORE` ise `rentalRepository.cancelPreparingRental(rentalId)` çağrılır (sonucu ne olursa olsun, çünkü rental zaten `PREPARING` değilse bu çağrının başarısız olması beklenir ve kullanıcı yine de geri gönderilmelidir), ardından `NavigateBack` effect'i gönderilir; `mode == AFTER` durumunda iptal çağrısı yapılmadan doğrudan `NavigateBack` gönderilir (rental zaten aktif, iptal edilecek bir şey yok). `VehicleConditionScreen.kt`: ekrandaki geri ok butonu artık doğrudan `onBack` yerine `onIntent(BackClicked)` çağırıyor; `androidx.activity.compose.BackHandler` eklendi ki sistem/donanım geri tuşu da aynı intent'i tetiklesin (önceden bu tuş hiç yakalanmıyor, Navigation Compose varsayılanına düşüyordu). `VehicleConditionRoute` artık `onBack`'i `Screen`'e vermiyor, yalnızca `NavigateBack` effect'i geldiğinde çağırıyor.

- Kapsam Dışı Bırakılan (bilinçli): Bu düzeltme yalnızca ileri yönlü davranışı değiştirir; bu değişiklikten önce zaten `PREPARING` durumunda asılı kalmış mevcut kiralama kayıtlarını geriye dönük temizlemez. `ActiveRentalViewModel`'in `rental.status`'u hiç kontrol etmemesi (aynı semptomun ikinci ayağı — kullanıcı Home üzerinden asılı kalmış kiralamaya ulaşıp "bitir" dediğinde backend'in "yalnızca aktif kiralamalar bitirilebilir" hatası vermesi) ayrı bir partide ele alınacaktır.

### ActiveRental Ekranı — rental.status Kontrolü Eklendi (Düzeltme)

- Karar: Kullanıcı ayrıca şunu bildirdi: rezervasyonu tamamlayıp sürüş öncesi foto ekranında hiç foto yüklemeden uygulamayı kapatıp tekrar açtığında, "sürüş başlamış" gibi görünen bir Aktif Kiralama ekranıyla karşılaşıyordu. Kök neden: `ActiveRentalViewModel.loadRentalStart()`, `getRentalDetails`'ten dönen `rental.status` alanını hiç okumuyordu; Home ekranı `GET /rentals/active` üzerinden `PREPARING` durumundaki (foto/`start` hiç tamamlanmamış) bir kiralamayı da döndürüp kullanıcıyı doğrudan bu ekrana yönlendirebiliyordu, ekran da bunu ayırt etmeden tam işlevli (sayaç çalışan, harita açık) bir "aktif sürüş" gibi gösteriyordu. Bu, aynı zamanda Parti 1'de ele alınan semptomun ikinci ayağıydı: geri tuşuyla değil, uygulamayı kapatıp açarak da aynı asılı `PREPARING` duruma ulaşılabiliyordu.

- Son Güncelleme Tarihi: 19.07.2026

- Uygulama: `ActiveRentalContract.kt`'ye `ActiveRentalEffect.RedirectToVehicleConditionBefore(rentalId, vehicleId, brand, model, plate)` eklendi. `ActiveRentalViewModel.loadRentalStart()`, `rental.status != "ACTIVE"` ise sayaç/plan/startFee state'ini hiç güncellemeden bu effect'i gönderip erken çıkıyor (`return@launch`). `ActiveRentalScreen.kt`'ye yeni bir `onRedirectToVehicleConditionBefore` parametresi eklendi ve effect toplayıcıda bu effect karşılanıyor. `RencarNavHost.kt`'de bu callback, Araç Durumu ekranını `mode = "BEFORE"` ile (durationSeconds/distanceMeters "0", pricePerDay "0.0" — bu bilgiler ActiveRentalUiState'te zaten tutulmuyor, sunucudan tekrar sorulacak) açıp `popUpTo(ROUTE_HOME)` ile geri yığını temizliyor.
...
- Backlog (Faz 2+, bu partinin kapsamı dışında, ayrı dallarda ele alınacak): `RentalRepository` (`RentalResponseDto`/`ActiveRentalResponseDto`/`PayRentalDto`/`VehicleLocationPoint` — `ReservationViewModel`, `ActiveRentalViewModel`, `PaymentCheckoutViewModel`, `TripSummaryViewModel`, `VehicleConditionViewModel`, `HistoryRepositoryImpl` tüketiyor), `AuthRepository.getMe()` (`UserResponseDto`), `WalletRepository` (`WalletDtos.kt`). Bu ekranlarda DTO tipi çoğunlukla açıkça import edilmediğinden (alan bazlı tüketim) sızıntı daha örtük; her biri ayrı bir domain modeli + repository imzası değişikliği gerektirir.

- Faz 2 Tamamlandı (20.07.2026): `domain/model/Reservation.kt` eklendi (yalnızca `id`; `status`/`expiresAt`/`remainingSeconds` şu an hiçbir ekranda okunmuyor, kullanılmayan alan eklenmedi). `ReservationRepository`/`ReservationRepositoryImpl` domain modelini döndürüyor. `ReservationViewModel` dönen değeri yalnızca null kontrolü için kullandığından değişmedi.

- Faz 3 Tamamlandı (20.07.2026): `domain/model/User.kt` eklendi (`fullName`/`phone`/`role`; `id`/`email` kullanılmadığından eklenmedi). `AuthRepository.getMe()` domain modelini döndürüyor. `WalletRepository` incelendi, zaten kendi domain tiplerini (`PaymentCard`/`WalletTransaction`) döndürdüğünden backlog'dan çıkarıldı. Kalan tek parça `RentalRepository`.
